...
for i in range(0,len(actpoints)):
	...
	for j in range(0,len(realpoints)):
		...
		if (abs(actpoints[i,0] - realpoints[j,0]) <= 0.04 and abs(actpoints[i,1] - realpoints[j,1]) <= 0.04):
			l.append(realpoints[j,2])

	actpoints[i,2] = np.amax(l)

	if((actpoints[i,2]>= 0.05 and actpoints[i,2] <= 0.2)):
		c = 'c'
	else:
		if((actpoints[i,2] >= -0.1 and actpoints[i,2] <= -0.065) or actpoints[i,2] == -0.19):
			c = 'b'
		else:
			c = 'j'
...
cv2.imshow('image',th2c)
cv2.imwrite("recogzie.png",th2c)
