...
class gen_path(object):
    ...
    def master_path(self):  
        ...
        for i in range(0,len(self.path)):
          self.isobstacle(list(self.path[i]),list(self.path[i+1]),i)
    ...
    def isobstacle(self,p1,p2,path_pos):
        p1 = np.array(p1[:2])
        p2 = np.array(p2[:2])
        ...
        for i,p3 in enumerate(self.node_data):
            ...
            if(p3[3]=='c'or p3[3]=='j'):
                ...
                if (d < self.threshold):
                    print i,d
                    x4 = p3[0] - (0.1)* (p1[1]-p2[1])
                    y4 =  p3[1] + 0.1 * (p1[0]-p2[0])
                    self.modify_path(path_pos,x4,y4)
...
#   target = node_data[1]
    ...
    cleaning_bot = gen_path(node_data,node_id,origin,target,threshold=0.1)
